...
#
# Derived from:
#   https://tools.ietf.org/id/draft-ietf-sidrops-aspa-verification-07.txt
#   Copyright (c) 2020 IETF Trust
#                      and the persons identified as the document authors.
#
"""aspa.draft-07.algorithm Module."""
...
class Validator(metaclass=MetaValidator):
    """ASPA verification as defined in draft-sidrops-aspa-verification-07."""
    ...
    def validate(self, as_path: AsPath, neighbor_as, afi, neighbor_type):
        ...
        if neighbor_type in (Customer, Peer, IxpRouteServerClient):
            ...
            return self.check_downflow_path(flattened_path, neighbor_as, afi)
        elif neighbor_type is IxpRouteServer:
            return self.check_ix_path(flattened_path, neighbor_as, afi)
        else:
    ...
    def check_downflow_path(self, aspath, neighbor_as, afi):  # noqa: E901
        if len(aspath) == 0:
            return Invalid
